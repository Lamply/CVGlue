...
def color_calibrator(iap_data, **kwargs):
    ...
    mask = (mask - 127.0) / 128.0
    mask_bool = mask[..., 0] > 0.5
    color_mat = cvglue.color_calibration(
        iap_data[0][mask_bool].reshape(-1, 3), ref_img[mask_bool].reshape(-1, 3)
    )
...
B_set.pipeline(
    "./",
    processors,
    meta_file="metadata_calib.yml",
    img_dir="calib",
    anno_file="blank.json",
)
