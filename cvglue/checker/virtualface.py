...
class virtual_face_checker(base_checker):
    def __init__(
        self, max_threshold=0.6, min_threshold=0.4, detect_face=False, verbose=False
    ):
        super().__init__()
        device = torch.device(
        ...
        self.facenet = (
            FaceFeatures("model_mobilefacenet").to(device).requires_grad_(False)
        )
        ...
        for fake_face in make_dataset(os.path.join(InsightFace_dir, "virtual_faces")):
            self.template += [
                self.trans(image=np.array(Image.open(fake_face)))["image"]
                .unsqueeze(0)
                .to(device)
            ]
