...
class SegmentAnything:
    def __init__(self, auto_mode=False, crf=False, crf_config=None, **kwargs):
        ...
        self.predictor = (
            SamAutomaticMaskGenerator(self.sam) if auto_mode else SamPredictor(self.sam)
        )
    ...
    def detect_manual(
        self, rgb_image, input_points=None, input_labels=None, input_boxes=None
    ):
        ...
        if self.crf:
            cat_logits = np.concatenate(
                [1.0 - raw_mask[None, ...], raw_mask[None, ...]],
                dtype=np.float32,
                axis=0,
            )
