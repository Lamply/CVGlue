...
class HeadPoseDetector:
    def __init__(self):
        ...
        self.sess = onnxruntime.InferenceSession(
            os.path.join(weights_path, "weights/fsanet.onnx"),
            providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        )
        self.sess2 = onnxruntime.InferenceSession(
            os.path.join(weights_path, "weights/fsanet_var.onnx"),
            providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        )
        self.sess3 = onnxruntime.InferenceSession(
            os.path.join(weights_path, "weights/fsanet_noS.onnx"),
            providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        )
    ...
    def detect(self, img, face_box):
        ...
        face_roi = cv2.normalize(
            face_roi, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX
        )
        ...
        yaw, pitch, roll = np.mean(
            np.vstack((res1, res2, res3)), axis=0, dtype=np.float64
        )
