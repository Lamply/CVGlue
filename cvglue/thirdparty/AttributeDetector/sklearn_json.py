...
def deserialize_csr_matrix(
    csr_dict, data_type=np.float64, indices_type=np.int32, indptr_type=np.int32
):
...
def serialize_svr(model):
    ...
    if isinstance(model.support_vectors_, sp.sparse.csr_matrix):
        serialized_model["support_vectors_"] = serialize_csr_matrix(
...
def serialize_random_forest(model):
    serialized_model = {
        ...
        "classes_": model.classes_.tolist(),
        "estimators_": [
            serialize_decision_tree(decision_tree)
            for decision_tree in model.estimators_
        ],
        "params": model.get_params(),
    }
    ...
    if "oob_score_" in model.__dict__:
        serialized_model["oob_score_"] = model.oob_score_
    if "oob_decision_function_" in model.__dict__:
        serialized_model["oob_decision_function_"] = (
            model.oob_decision_function_.tolist()
        )
