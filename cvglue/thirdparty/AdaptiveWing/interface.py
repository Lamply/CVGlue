...
class AdaptiveWing:
    def __init__(self):
        PRETRAINED_WEIGHTS = os.path.join(
            os.environ["TORCH_HOME"], "WFLW_4HG.pth"
        )  # too large, not included in package
        GRAY_SCALE = False
        HG_BLOCKS = 4
        END_RELU = False
        NUM_LANDMARKS = 98
    ...
    def to_boundaries(landmarks_98):
        ...
        # ROI
        boundaries["nose"] = np.concatenate(
            [boundaries["nose_ridge"], boundaries["nose_bot"]]
        )
        boundaries["eye_left"] = np.concatenate(
            [boundaries["eyelid_left"], boundaries["eye_center_left"][np.newaxis, ...]]
        )
        boundaries["eye_right"] = np.concatenate(
            [
                boundaries["eyelid_right"],
                boundaries["eye_center_right"][np.newaxis, ...],
            ]
        )
        boundaries["mouth"] = np.concatenate(
            [boundaries["lip_outer"], boundaries["lip_inner"]]
        )
        # boundaries['ROI'] = np.concatenate([boundaries['nose'], boundaries['eye_left'], boundaries['eye_right'], boundaries['mouth']])

        # Delaunay triangulation
        boundaries["triangulation"] = np.concatenate(
            [
                boundaries["cheek"][::2],
        ...
        # contours
        boundaries["ROI"] = np.concatenate(
            [
                boundaries["upper_left_eyelid"],
                boundaries["upper_right_eyelid"],
                boundaries["lower_outer_lip"][::-1],
            ]
        )
    ...
    def cal_eyelid_closure(cls, lands):
        ...
        eyelid_res_left = [
            np.hypot(*(bd["eyelid_left"][8 - i] - bd["eyelid_left"][i]))
            for i in range(1, 4)
        ]
        ...
        eyelid_res_right = [
            np.hypot(*(bd["eyelid_right"][8 - i] - bd["eyelid_right"][i]))
            for i in range(1, 4)
        ]
