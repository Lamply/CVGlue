        ...
        def process_func(iap_data):
            ...
            for i, face in enumerate(iap_data[1]["faces"]):
                ...
                if method == crop_face_v3.__name__:
                    crop_img, warp_mat = crop_face_v3(
                        iap_data[0], face["key_points"], face["landmarks"], **params
                    )
                elif method == crop_face_v4.__name__:
                    crop_img, warp_mat = crop_face_v4(
                        iap_data[0], face["face_box"], face["key_points"], **params
                    )
                elif method == norm_crop.__name__:
                    crop_img, warp_mat = norm_crop(
                ...
                cropped_face["face_box"] = list(
                    warp_face_box(
                        face["face_box"], face["landmarks"], warp_mat
                    ).reshape(-1)
                )
                cropped_face["key_points"] = list(
                    warp_landmarks(face["key_points"], warp_mat).reshape(-1)
                )
                cropped_face["landmarks"] = warp_landmarks(
                    face["landmarks"], warp_mat
                ).tolist()
