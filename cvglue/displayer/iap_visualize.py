    ...
    if anno_exists(iap_data[1], "faces"):
        for face in iap_data[1]["faces"]:
            ...
            if anno_exists(face, "key_points") and "key_points" not in disable_domain:
                ...
                cv2.circle(
                    img_disp, (key_points[0], key_points[1]), 1, (0, 0, 255), thickness
                )
                cv2.circle(
                    img_disp,
                    (key_points[2], key_points[3]),
                    1,
                    (0, 255, 255),
                    thickness,
                )
                cv2.circle(
                    img_disp,
                    (key_points[4], key_points[5]),
                    1,
                    (255, 0, 255),
                    thickness,
                )
                cv2.circle(
                    img_disp, (key_points[6], key_points[7]), 1, (0, 255, 0), thickness
                )
                if len(key_points) > 8:
                    cv2.circle(
                        img_disp,
                        (key_points[8], key_points[9]),
                        1,
                        (255, 0, 0),
                        thickness,
                    )
            if anno_exists(face, "landmarks") and "landmarks" not in disable_domain:
                img_disp = render_face(
                    img_disp,
                    face["landmarks"],
                    thickness=max(img_disp.shape[0], img_disp.shape[1]) // 300,
                )
            if (
                anno_exists(face, "gender")
                and anno_exists(face, "age")
                and "genderage" not in disable_domain
            ):
