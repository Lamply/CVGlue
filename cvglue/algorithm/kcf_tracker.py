...
def rearrange(img):
    ...
    img_[0:yh, 0:xh], img_[yh : img.shape[0], xh : img.shape[1]] = (
        img[yh : img.shape[0], xh : img.shape[1]],
        img[0:yh, 0:xh],
    )
    img_[0:yh, xh : img.shape[1]], img_[yh : img.shape[0], 0:xh] = (
        img[yh : img.shape[0], 0:xh],
        img[0:yh, xh : img.shape[1]],
    )
    return img_
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
    ...
    res = img[
        cutWindow[1] : cutWindow[1] + cutWindow[3],
...
class KCFTracker:
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.0):
        ...
        if inithann:
            ...
            if self._hogfeatures:
                self._tmpl_sz[0] = (
                    int(self._tmpl_sz[0]) // (2 * self.cell_size) * 2 * self.cell_size
                    + 2 * self.cell_size
                )
                self._tmpl_sz[1] = (
                    int(self._tmpl_sz[1]) // (2 * self.cell_size) * 2 * self.cell_size
                    + 2 * self.cell_size
                )
            else:
        ...
        if self._hogfeatures:
            ...
            FeaturesMap = (
                mapp["map"]
                .reshape((self.size_patch[0] * self.size_patch[1], self.size_patch[2]))
                .T
    ...
    def update(self, image):
        ...
        if self.scale_step != 1:
            ...
            if (
                self.scale_weight * new_peak_value1 > peak_value
