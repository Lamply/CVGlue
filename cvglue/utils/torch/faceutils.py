...
def cal_ROI_AUC(
    pred, gt, face_anno, dscale=0.3, eye_mouth_only=True, return_curve=False
):
    ...
    if eye_mouth_only:
        contours = [
            np.int64(bounds["eyelid_left"]),
            np.int64(bounds["eyelid_right"]),
            np.int64(bounds["lip_outer"]),
        ]
    else:
        contours = [np.int64(bounds["ROI"])]
    fg_mask = generate_contours_mask(
        pred.shape[2], pred.shape[3], contours, blur=False, dsize=dsize
    )
    fg_mask_gpu = torch.from_numpy(fg_mask[np.newaxis, np.newaxis, :, :]).cuda(
        non_blocking=True
    )
    ...
    xs = (
        1 - np.logspace(0, 2, 100, endpoint=False) / 100
    )  # In case of higher metric matter
    xs = xs[::-1]
...
def crop_face_v3_tensor(
    img, keypoints, landmarks, rotate=True, output_size=None, **kwargs
):
    ...
    if no_img:
        ...
        crop_img = warpAffine(
            img, warp_mat, (img.shape[0], img.shape[1], warp_height, warp_width)
        )
        if antialias:
            crop_img = F.interpolate(
                crop_img, (output_size_h, output_size_w), mode="area"
            )
...
def norm_crop_tensor(img, keypoints, output_size=None, mode="arcface", **kwargs):
    ...
    if no_img:
        ...
        crop_img = warpAffine(
            img, M, (img.shape[0], img.shape[1], warp_size, warp_size)
        )
        if antialias:
