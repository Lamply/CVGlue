...
__all__ = [
    ...
    "warp_back",
    "warp_face_Delaunay",
    "warp_face_Delaunay_flow",
    "warp_face_box",
    "warp_face_landsmarks",
    "warp_landmarks",
    "warp_with_landmarks",
]
...
def cal_eye_dist(boundaries):
    return np.sqrt(
        np.sum((boundaries["eye_center_right"] - boundaries["eye_center_left"]) ** 2)
    )
...
def warp_landmarks(landmarks, warp_mat):
    ...
    rotated_landmark = np.matmul(
        np.insert(landmark_array, [2], 1, axis=-1), np.transpose(warp_mat)
    )
...
def warp_with_landmarks(
    img, landmarks, out_size, center, angle, scale=1.0, shift_xy=(0, 0)
):
...
def warp_back(
    img, src, warp_mat, mask_warp=True, remove_edge=False, keep_mask=False, **kwargs
):
    ...
    back_img = np.float32(
        cv2.warpAffine(
            img,
            cv2.invertAffineTransform(warp_mat),
    ...
    fg_mask = back_img[:, :, 3:]
    if remove_edge:
        fg_mask = cv2.morphologyEx(fg_mask, cv2.MORPH_ERODE, np.ones((7, 7)))[
            :, :, np.newaxis
        ]
...
def warp_face_Delaunay_flow(
    src_img, tgt_img, src_points, tgt_points, patch_warp=False, patch_factor=1.1
):
    ...
    x_grid, y_grid = np.meshgrid(
        np.arange(src_img.shape[1]), np.arange(src_img.shape[0])
    )
    ...
    for i, triangle in enumerate(triangulation.simplices):
        ...
        warped_grid = warp_triangle(
            xy_grid, src_triangle, tgt_triangle, shape=(out_shape[1], out_shape[0])
        )
...
def align_face(img, keypoints, output_size=None, rotate=True, **kwargs):
    ...
    if (
        face_lt[0] > img.shape[1]
        or face_lt[1] > img.shape[0]
        or face_rb[0] < 0
        or face_rb[1] < 0
    ):
    ...
    if antialias:
        aligned_img = cv2.resize(
            aligned_img, (output_size, output_size), interpolation=cv2.INTER_AREA
        )
...
def crop_face_v2(img, keypoints, landmarks):
    ...
    crop_img = cv2.warpAffine(
        img,
        warp_mat,
        (int(face_rect[2] - face_rect[0]), int(face_rect[3] - face_rect[1])),
    )
...
def crop_face_v3(img, keypoints, landmarks, rotate=True, output_size=None, **kwargs):
    ...
    border_value = kwargs.get("border_value", 0)
    antialias = kwargs.get("antialias", False)
    max_warp_size = kwargs.get(
        "max_warp_size", np.inf
    )  # performance setting, 512 is recommended
    ...
    if use_chin:
        ...
        shift_factor = (
            shift_factor if hp_dst / shift_factor > ms_dst else 1.0
        )  # need to slightly shift up face_rect in common case
    ...
    if output_size and not antialias:
        ...
        warp_height = (
            int(rect_height) if rect_height <= max_warp_size else max_warp_size
        )
        warp_width = (
            int(rect_width)
            if rect_height <= max_warp_size
            else int(max_warp_size / ratio)
        )
    ...
    if no_img:
        ...
        if antialias:
            crop_img = cv2.resize(
                crop_img, (output_size_w, output_size_h), interpolation=cv2.INTER_AREA
            )
...
def crop_face_v4(
    ...
    warp_mat[1, 2] -= dst_lt[1]
    crop_img = cv2.warpAffine(
        img, warp_mat, (output_size_w, output_size_h), flags=interp_method
    )
...
def get_hairline_point_cropv3(face_box, landmarks, rotate=True, debug=False):
    ...
    cheek_vector = right_cheek_corner - left_cheek_corner
    k = (
        -cheek_vector[0] / cheek_vector[1]
    )  # set minus because the image x-y axis is right-bottom, set k=(x / y) indicates rotation
...
def generate_face_mask(
    ...
    **kwargs,
    ...
    if include_forehead:
        if top_y == -1:
            top_center = get_hairline_point_cropv3(
                kwargs["face_box"], landmarks, rotate=False
            )
            top_y = top_center[1]
        else:
        ...
        boundaries["left_head"] = np.array(
            [bezier_func(t, top_center, top_left, left_cheek_top) for t in inter_t]
        )
        boundaries["right_head"] = np.array(
            [bezier_func(t, right_cheek_top, top_right, top_center) for t in inter_t]
        )
        contours = np.int64(
            np.concatenate(
                (
                    boundaries["cheek"],
                    boundaries["right_head"],
                    top_center.reshape(-1, 2),
                    boundaries["left_head"],
                )
            )
        )
    else:
        contours = np.int64(
            np.concatenate(
                (
                    boundaries["cheek"],
                    boundaries["eyebrow_right"][::-1],
                    boundaries["eyebrow_left"][::-1],
                )
            )
        )
    ...
    if not fast:
        ...
        blur = False
        dilate = False
    fg_mask = generate_contours_mask(
        src_height, src_width, [contours], blur=blur, dilate=dilate, **kwargs
    )
        ...
        np.int64(
            np.concatenate(
                (
                    close_s1,
                    close_s2,
                    cheek_points[depth : num_cp - depth],
                    close_e1,
                    close_e2,
                )
            )
