...
__all__ = [
    ...
    "cal_residual_image",
    "color_calibration",
    "color_calibration_v2",
    "crop_center",
    "crop_rect",
    "crop_roi_points_image",
    "cvt_box_format",
    "cvt_color",
    "fill_roi_image",
    "gaussian_kernel",
...
def cal_residual_diff(
    img_a, img_b, method="", fg_mask=None, morpho="", morpho_size=None
):
...
def pad_rect(rect, padding_scale=None, padding_fix=None, box_wh_fix=None):
    ...
    if padding_fix is not None:
        padding_fix = np.array(
            [-padding_fix[0], -padding_fix[1], padding_fix[2], padding_fix[3]]
        )
...
def crop_roi_points_image(
    image, roi_points, padding_scale=None, padding_fix=None, box_wh_fix=None
):
    """
    ...
    box = pad_rect(
        box, padding_scale=padding_scale, padding_fix=padding_fix, box_wh_fix=box_wh_fix
    )
...
def color_calibration(roi_img, roi_ref):
    ...
    trans_mat = np.matmul(
        np.matmul(
            np.linalg.pinv(np.matmul(poi_img_mat.transpose(), poi_img_mat)),
            poi_img_mat.transpose(),
        ),
        poi_ref_mat,
    )
    return trans_mat
...
def color_calibration_v2(roi_img, roi_ref, print_inlier_ratio=True):
    ...
    ransac = linear_model.RANSACRegressor(
        estimator=linear_model.Ridge(alpha=0.001), min_samples=norm_x.shape[1] + 1
    )
...
def apply_calibration(img, trans_mat):
    return np.uint8(
        np.clip(
            np.matmul(
                np.insert(img.reshape(-1, 3), [3], 1, axis=-1), trans_mat
            ).reshape(img.shape),
            0,
            255,
        )
...
def resize_like(src_img, dst_img, interp=cv2.INTER_LINEAR):
    resize_img = cv2.resize(
        src_img, (dst_img.shape[1], dst_img.shape[0]), interpolation=interp
    )
    return resize_img


def resize_scale(
    src_img, scale=1.0, align_length=0, align_flag="height", interp=cv2.INTER_LINEAR
):
    ...
    if scale == 1.0 and align_length == 0:
        ...
        if align_flag == "height":
            resize_img = cv2.resize(
                src_img,
                (int(src_img.shape[1] * align_length / src_img.shape[0]), align_length),
                interpolation=interp,
            )
        elif align_flag == "width":
            resize_img = cv2.resize(
                src_img,
                (align_length, int(src_img.shape[0] * align_length / src_img.shape[1])),
                interpolation=interp,
            )
        else:
...
def resize_alignrect(src_img, align_rect, bounding=False):
    ...
    src_img = resize_scale(
        src_img, align_length=align_length, align_flag=align_flag, interp=interp
    )
    return src_img
...
def resize_fix(
    ...
    dst_w=0,
    flag="pad",
    pad_method=cv2.BORDER_CONSTANT,
    pad_value=0,
    position="center",
):
    ...
    if flag == "fix":
        if dst_w == 0:
            ...
            resize_img = cv2.resize(
                src_img, (int(dst_w), int(dst_h)), interpolation=interp
            )
        return resize_img
    ...
    if src_ratio > dst_ratio:
        if flag == "crop":
            ...
            if position == "center":
                fixed_img, _ = pad_center(
                    resize_img, (dst_h, dst_w), border_mode=pad_method, value=pad_value
                )
            elif position == "topleft":
                fixed_img = cv2.copyMakeBorder(
                    resize_img,
                    0,
                    0,
                    0,
                    dst_w - resize_img.shape[1],
                    pad_method,
                    value=pad_value,
                )
            else:
...
def gaussian_kernel(sigma, ksize=None):
    ...
    if isinstance(ksize, int):
        ...
        ksize_x = ksize[0]
    weight = np.matmul(
        cv2.getGaussianKernel(ksize_y, sigma), cv2.getGaussianKernel(ksize_x, sigma).T
    )
    return weight
