...
__all__ = [
    ...
    "apply_feather",
    "cal_residual_mask",
    "calculate_similarity_to_shape",
    "crop_contours",
    "detect_white_edge_padding",
    "edge_soft_mask",
    "generate_contours_mask",
    "render_mask",
    "select_mask_area",
    "select_mask_regoin",
    "soft_threshold",
...
def cal_residual_mask(
    img_a, img_b, method="l1sum", threshold=50, morpho="", morpho_size=None
):
...
def generate_contours_mask(
    src_height, src_width, contours, blur=True, dilate=True, **kwargs
):
    """
    Notice:
...
def render_mask(
    ...
    mask = (
        cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)[:, :, None]
        if mask.shape[-1] == 3
        else mask
    )
    if matting:
        ...
        cover_mask = (mask > thres) * alpha if alpha > 0.0 else mask
...
def select_mask_area(mask, select_area=None, largest=False, method="pixel"):
    ...
    if select_area:
        _ = [
            cv2.fillPoly(mask, [contours[0][i]], (0))
            for i in range(len(cons_area))
            if cons_area[i] < select_area[0] or cons_area[i] > select_area[1]
        ]
    elif largest:
        _ = [
            cv2.fillPoly(mask, [contours[0][i]], (0))
            for i in range(len(cons_area))
            if cons_area[i] < max(cons_area)
        ]
    return mask
...
def detect_white_edge_padding(img, feather=False, **kwargs):
    ...
    pad_mask, (pl, pt) = pad_center(
        thres_mask, (thres_mask.shape[0] * 2, thres_mask.shape[1] * 2), value=1
    )
